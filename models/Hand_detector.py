...
import numpy as np
import os
hand_object_detector_path = './detectors'
...
class Hand_track_Detector(wrist_detect):
    ...
    def __load_hand_detector(self,model_path,device):
        ...
        # cfg.MODEL.WEIGHTS = 'extra_data/hand_module/hand_detector/model_0529999.pth'  # add models weight here
        # cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = 0.4  # 0.3 , use low thresh toincrease recall
        # self.hand_detector = DefaultPredictor(cfg)

    def __get_raw_hand_bbox(self, img, index, f_name):
        bbox = self.run_each_dataset(self.hand_detector, img, index, f_name)
        bboxes = np.array(bbox)
        # bbox_tensor = self.hand_detector(img)['instances'].pred_boxes
        # bboxes = bbox_tensor.tensor.cpu().numpy()
        return bboxes

    def run_each_dataset(self,model, img, index, f_name):
        if f_name:
            self.det_list_all = []
            self.max_id = 0
            self.det_list_all = []
    ...
    def detect_hand_bbox(self, img, index, f_name):

        # get raw hand bboxes
        raw_hand_bboxes = self.__get_raw_hand_bbox(img, index, f_name)
        ...
        if index==0:
            ...
            # for i in wrist:
            #     for p in i[[2,5]]:
            #         cv2.circle(img, (int(p[0]),int(p[1])),4,(255,13,155),3)
            # for v in wrist_bbox:
            #     if v[-1]:
            #         lx,ly,lw,lh = v[:-1]
            #         cv2.rectangle(img, (int(lx), int(ly)), (int(lx + lw), int(ly + lw)), (255, 255, 0), 1)
            #     else:
            #         lx1,ly1,lw1,lh1 = v[:-1]
            #         cv2.rectangle(img, (int(lx1), int(ly1)), (int(lx1+lw1), int(ly1+lw1)), (0,255,0),1)
            # for i in hand_bbox_list:
            #     x,y,x2,y2,_ = i['left_hand']
            #     x1,y1,x3,y3,_ = i['right_hand']
            #     cv2.rectangle(img,(int(x),int(y)),(int(x2),int(y2)), (255,255,0),5)
            #     cv2.rectangle(img,(int(x1),int(y1)),(int(x3),int(y3)), (0,255,0),5)
            # cv2.imwrite("/mnt/dms/KTW/data/wrist2.jpg",img)
    ...
    # def simple_dist(self,raw_bbox):
    #     tmp= np.array([None,]*len(raw_bbox))
    #     for idx, hands in enumerate(raw_bbox):
    #         c_x = (hands[0]+hands[2])/2
    #         tmp[idx] = c_x
    #     index = np.argmax(tmp)
    #     index1 = np.argmin(tmp)
    #     return raw_bbox[index], raw_bbox[index1]
    ...
    def detect_hand_bbox_openpose(self, img, body_pose_list):
        ...
        # for body_pose in body_pose_list[0].reshape(-1,6,3):
            hands = []
...
class Handtrack(object):
    ...
    def detect_hand_bbox(self,img_bgr,index,f_name):
        output = self.model.detect_hand_bbox(img_bgr,index, f_name)
