...
def bbox_preprocessing(img,bbox,flip=None):
    transform = transforms.ToTensor()
    original_img_height, original_img_width = img.shape[:2]
    hand_bbox = process_bbox(bbox, original_img_width, original_img_height)
    hand_bbox = list(map(int, hand_bbox))
    hand_img, img2bb_trans, bb2img_trans = generate_patch_image(img, hand_bbox, 1.0, 0.0, flip,
                                                                 cfg.input_hand_img_shape)
    hand_img = transform(hand_img.astype(np.float32)) / 255
    hand_img = hand_img.cuda()[None, :, :, :]
    return hand_img, hand_bbox
