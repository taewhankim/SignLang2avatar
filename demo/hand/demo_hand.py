...
cfg.set_args(args.gpu_ids, 'hand')
cfg.handsign_folder_path = '/mnt/dms/HAND_SIGN/val_demo'

# input_list = ['속초 해수욕장 속초 해수욕장은 속초를 대표하는 관광지입니다', '깨끗한 수질과 편의시설이 잘 갖추어져 있습니다', '해수욕장 개장과 동시에 많은 사람들이 방문하는 곳입니다', '이 곳에는 공원과 꽃길 그리고 소나무 숲 사이로 난 산책로가 있습니다']
# input_list = ['전통 돌하르방도 있고 고인돌도 있어 제주의 옛 문화를 직접 눈으로 보며 체험할 수 있습니다']
# input_list = ['전라도 순천 낙안읍성','순천 낙안읍성은 옛 모습을 그대로 간직하고 있는 전통마을입니다','순천 여행 시 꼭 들러야 하는 명소입니다','전통 한옥들이 있으며 현재 실제 주민들이 거주 중입니다']
if args.input_text:
    hand_text = []
    hand_videos = []
...
for i, batch in enumerate(tqdm(testdata, dynamic_ncols=True)):
    # try:
        ...
        tmp_result = cont_dict(args,result_data,tmp,tmp_result)
