...
from .flownet import flownets_bn
import copy
...
class BiFPN(Backbone):
    ...
    def __init__(
        ...
        self.mix_list = nn.ModuleList()
        out_channels_list = [128, 256, 512, 512, 1024]
        # in_channels_list = [self._out_feature_channels[name] for name in self._out_features]
        in_channels_list = [bottom_up_output_shapes[name].channels*2 for name in in_features]
        for i in range(len(self._out_features)):
            out_mix_channels = out_channels_list[i]
            in_mix_channels = in_channels_list[i]
            self.mix_list.append(Conv2d(in_mix_channels, out_mix_channels, kernel_size=3, padding=1,
                 bias=True, norm=get_norm(norm, out_mix_channels), activation=nn.ReLU(inplace=True),))

        # self.offset = flownets_bn()
        self.device = comm.get_local_rank()
        self.offset = flownets_bn(None, comm.get_local_rank())
        self.dual_bifpn = SingleBiFPN(in_channels_list, out_channels, norm)
    ...
    def forward(self, x):
        bottom_up_features = self.bottom_up(x['cur'])
        ori_feats = feats = [bottom_up_features[f] for f in self.in_features]
        pred_offsets = offset_feats = cur_hm = None
        if 'pre_ori_feats' in x:
            pre_feats = x['pre_ori_feats']
            pre_hm = x['heatmap'] 

        if 'pre_ori_feats' in x:
            concat_feats = [torch.cat((pre_feats[f], ori_feats[f]), dim=1) for f in range(len(self.in_features))]
            offset_feats = self.dual_bifpn(concat_feats)

            for bifpn in self.repeated_bifpn[1:]:
                offset_feats = bifpn(offset_feats)
            flow_feats = [mix(feat) for mix, feat in zip(self.mix_list, concat_feats)]

            pred_offsets = self.offset(flow_feats)
            pred_offsets_reshape = [pred_offset.permute(0, 2, 3, 1) for pred_offset in pred_offsets]
            cur_hm = [F.grid_sample(h, offset, mode="bilinear", padding_mode="border", align_corners=True).detach()
                      for h, offset in zip(pre_hm, pred_offsets_reshape)]
        ...
        for i, bifpn in enumerate(self.repeated_bifpn):
            if 'pre_ori_feats' in x:
                if i == 0:
                    feats = [f * (1 + ch) + pf * ph
                             for f, pf, ph, ch in zip(ori_feats, pre_feats, pre_hm, cur_hm)]
                else:
                    feats = [f * (1 + ch) for f, ph, ch in zip(feats, pre_hm, cur_hm)]
            feats = bifpn(feats)
        
        feats = dict(zip(self._out_features, feats))

        if 'pre_ori_feats' in x:
            return feats,ori_feats, pred_offsets, offset_feats, cur_hm
        else:
            return feats, ori_feats, None, None, None
...
def build_siamese_fcos_resnet_bifpn_backbone(cfg, input_shape: ShapeSpec):
    """
    Args:
...
@BACKBONE_REGISTRY.register()
def build_siamese_p37_fcos_dla_bifpn_backbone(cfg, input_shape: ShapeSpec):
    """
    Args:
        cfg: a detectron2 CfgNode
    Returns:
