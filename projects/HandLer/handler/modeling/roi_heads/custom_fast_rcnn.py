...
from FastRCNNOutputs import FastRCNNOutputs
...
class CustomFastRCNNOutputLayers(FastRCNNOutputLayers):
    ...
    # def trk_inference(self, predictions, proposals, trk):
    #     """
    #     Args:
    ...
    #     scores = self.predict_probs(predictions, proposals)
    #     image_shapes = [x.image_size for x in proposals]
    #     boxes, scores = boxes[0][trk], scores[0][:, 0][trk]
    #     result = Instances(image_shapes[0])
    #     result.pred_boxes = Boxes(boxes)
    #     result.scores = scores
    #     result.pred_classes = torch.zeros(boxes.shape[0]).long().cuda()
    #     return [result]
...
def trk_inference(predictions, proposals, trk):
    boxes, scores = boxes[0][trk], scores[0][:, 0][trk]
    result = Instances(image_shapes[0])
    result.pred_boxes = Boxes(boxes)
    result.scores = scores
