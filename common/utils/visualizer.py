...
from renderer import set_rasterizer, SRenderY
...
class Visualizer(object):
    ''' visualizer
    '''
    def __init__(self, render_size=1024, config=None, device='cuda:0', part='body', background=None, rasterizer_type='standard', args=None):
        if config is None:
            self.cfg = cfg
        else:
            self.cfg = config
        ...
        # if args.texmap_path:
        #     self.smplx_texture = imread(args.texmap_path)/255.
        # else:
    ...
    def light_albedo(self, opdict):
        ...
        ## coarse_normals -> [batch size, number of vertices, 3]
    ...
    def render_results(self, opdict, input_images, overlay=False, light_on=True, moderator_weight=None, use_deca=False, background=None):
        ...
        shape_images, normal_images, grid = self.render.render_shape(opdict['vertices'], opdict['transformed_vertices'], background=None, return_grid=True, h=h, w=w)
        visdict={
            'inputs': input_images,
        ...
        render_ops = self.render(opdict['vertices'], opdict['transformed_vertices'], uvmap, background=None, lights=lights, h=h, w=w)
