...
class KobertCRF(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, tags=None):
        # 이 forward 는 batch 단위로 진행되지 않음 
        # bert model 이 옛날것으로, bert의 encoder 12개 layer output을 모두 제공한다. 
        # 이때 맨 마지막이 가장 마지막 encoder layer의 output이다. 
        ...
        # 맨 마지막이 가장 마지막 encoder layer의 output이다. 
        last_encoder_layer = outputs[0][-1]
