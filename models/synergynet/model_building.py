...
from models.synergynet.params import ParamsPack
param_pack = ParamsPack()

# import resnet_backbone
from models.synergynet import mobilenetv1_backbone
from models.synergynet import mobilenetv2_backbone
# from backbone_nets import ghostnet_backbone
from models.synergynet.pointnet_backbone import MLP_for, MLP_rev
from models.synergynet.loss_definition import ParamLoss, WingLoss

# from backbone_nets.ResNeSt import resnest50, resnest101
import time
from models.synergynet.inference import predict_sparseVert, predict_denseVert, predict_pose, crop_img
...
class I2P(nn.Module):
	def __init__(self, arch):
		...
		self.arch = arch
		# backbone definition
		if 'mobilenet_v2' in self.arch:
			self.backbone = getattr(mobilenetv2_backbone, arch)(pretrained=False)
		elif 'mobilenet' in self.arch:
			self.backbone = getattr(mobilenetv1_backbone, arch)()
		# elif 'resnet' in self.arch:
		# 	self.backbone = getattr(resnet_backbone, arch)(pretrained=False)
		# elif 'ghostnet' in self.arch:
		# 	self.backbone = getattr(ghostnet_backbone, arch)()
...
class SynergyNet(nn.Module):
	def __init__(self, img_size, arch):
		super(SynergyNet, self).__init__()
		self.triangles = sio.loadmat('/mnt/dms/KTW/synergynet/weights/3dmm_data/tri.mat')['tri'] -1
