...
class CenterNet(nn.Module):
    ...
    def forward(self, images, features_dict, gt_instances, return_hm=False, offset_hm_per_level=None):
        ...
        if not self.training:
            ...
            if offset_hm_per_level is not None:
                # import pdb ; pdb.set_trace()
                _, _, offset_hm = self._flatten_outputs(None, None, offset_hm_per_level)
            else:
                offset_hm = None
            losses = self.losses(
                pos_inds, labels, reg_targets, flattened_hms, 
                logits_pred, reg_pred, agn_hm_pred, offset_hm)
            ...
            if return_hm:
                return proposals, losses, agn_hm_pred_per_level
            return proposals, losses


    def losses(
        self, pos_inds, labels, reg_targets, flattened_hms,
        logits_pred, reg_pred, agn_hm_pred, offset_hm=None):
        '''
        Inputs:
            pos_inds: N
            labels: N
        ...
        if self.with_agn_hm and offset_hm is not None:
            # cat_agn_heatmap = flattened_hms.max(dim=1)[0] # M
            agn_pos_loss, agn_neg_loss = binary_heatmap_focal_loss_jit(
                offset_hm, cat_agn_heatmap, pos_inds,
                alpha=self.hm_focal_alpha, 
                beta=self.hm_focal_beta, 
                gamma=self.loss_gamma,
                sigmoid_clamp=self.sigmoid_clamp,
            ...
            agn_pos_loss = self.pos_weight * agn_pos_loss / num_pos_avg
            agn_neg_loss = self.neg_weight * agn_neg_loss / num_pos_avg
            losses['loss_centernet_off_pos'] = agn_pos_loss
            # losses['loss_centernet_off_neg'] = agn_neg_loss
    ...
    def _flatten_outputs(self, clss, reg_pred, agn_hm_pred):
        ...
        clss = cat([x.permute(0, 2, 3, 1).reshape(-1, x.shape[1]) \
            for x in clss], dim=0) if clss is not None and clss[0] is not None else None
        reg_pred = cat(
            [x.permute(0, 2, 3, 1).reshape(-1, 4) for x in reg_pred], dim=0) if reg_pred is not None else None
    ...
    def inference(self, images, clss_per_level, reg_pred_per_level, 
        ...
        return proposals, {}, agn_hm_pred_per_level
