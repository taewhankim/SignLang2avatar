...
def visualize_grid(visdict, savepath=None, size=224, dim=2, return_gird=True):
    '''
    ...
    grids = {}
    for key in visdict:
        img = visdict[key].transpose(2,0,1)
        visdict[key] = torch.tensor(img).to('cuda')
    ...
    grid_image = (grid.numpy().transpose(1,2,0).copy())[:,:,[2,1,0]]
