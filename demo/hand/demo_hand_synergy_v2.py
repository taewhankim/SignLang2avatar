...
for i, batch in enumerate(tqdm(testdata, dynamic_ncols=True)):
    # try:
        ...
        crop_head = cv2.imread(os.path.join(ch_path, "head_{}.png".format(i)))
        face_texture = synergynet(crop_head,n_face_bbox)
        face_texture = face_texture.astype(np.float64).copy()
...
for i , bat in enumerate(tqdm(tmp_result, dynamic_ncols=True)):
    # try:
        ...
        face_texture = bat['face_texture'].astype(np.uint8).copy()
    ...
    ###################
        ...
        # face_rendered_img2 = synergynet(crop_head)
        if i == 0:
            first_prin = fix_princpt
        else:
            fix_princpt = first_prin
        new_f_h,new_f_w = face_texture.shape[:2]
        ...
        #     first_size = face_texture.shape[:2]
        # else:
        #     new_face_bbox = first_bbox
        #     face_texture = cv2.resize(face_texture,(int(first_size[1]),int(first_size[0])),interpolation=cv2.INTER_AREA)
        # t_h, t_w = face_texture.shape[:2]
        # new_face_bbox = fix_princpt[0]- t_w/2, fix_princpt[1] - t_h/2, t_w, t_h
        # new_face_bbox = list(map(int, new_face_bbox))

        face_rendered_img = syn_imgaes(face_rendered_img.astype(np.uint8),face_texture,new_face_bbox)
